...
def process_all_videos(
    ...
    clip_duration
     ...
     # predefine TODO: remove the these options and the extract_frames function
    emb_type = 'video' # 
    N = 2
    # def extract_frames(video_path, image_output_dir, meta_output_dir, frames_per_second, date_time, local_timezone):
    ...
    #     mod = int(fps // frames_per_second)
    #     if mod == 0: 
    #         mod = 1
        
    #     logging.info(f"total frames {total_frames}, frames_per_second {frames_per_second}, mod {mod}")
    ...
    for i, each_video in enumerate(videos):
        ...
        logging.info(f"date_time of {video_path} being created : {date_time}")
        # Get the local timezone of the machine
        local_timezone = get_localzone()
        if emb_type == 'frame': # TODO: remove this option
            ...
            # })
            logging.info(f"bypass frame mode")
            pass
        elif emb_type == 'video':
            ...
            if clip_duration is not None and chunk_duration is not None and clip_duration < chunk_duration:
                ...
                for start_frame, end_frame, start_time, end_time in calculate_intervals(video_path, chunk_duration, clip_duration):
                    ...
                    # if selected_db == 'vdms': TODO remove 
                    # Localize the current time to the local timezone of the machine
                    #Tahani might not need this
                    current_time_local = date_time.replace(tzinfo=datetime.timezone.utc).astimezone(local_timezone)
